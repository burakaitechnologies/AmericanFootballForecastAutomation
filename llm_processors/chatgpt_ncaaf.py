"""ChatGPT/Tavily-based NCAAF prediction gatherer.
...
# System prompt for ChatGPT score extraction
SYSTEM_PROMPT = """You are a college football game prediction extractor and formatter.

Given raw web search results about a specific game matchup, you must identify any predicted or expected scores mentioned in the text, average them if multiple appear, and return a clean JSON structure with the following fields:
...
- Use integer values.
- If no score is found, set both scores to 24 and 21 respectively.
- Do not include any commentary or extra text.
- Only extract scores that look realistic (10–70 range).
...
QUERY_VARIATIONS = [
    "{away_team} vs {home_team} predicted score",
]
...
def extract_scores_with_chatgpt(
    ...
    content = None
    try:
        user_prompt = f"""Extract predicted scores for the following game from the web search results below.
...
def run_chatgpt_ncaaf(config: Optional[Config] = None) -> bool:
    """
    Main function to gather predictions using Tavily and ChatGPT.
    ...
    # Load games from sheets (NCAAF)
    sheets_file = config.get_games_scraped_path("sheets_games.json", league="ncaaf")
    ...
    logger.info(f"Processing {len(games)} games from sheets")
    ...
    if missing_rows:
        ...
        matched_file = config.get_data_path("matched_games.json", league="ncaaf")
        matched_data = {}
        ...
        for row_num in missing_rows:
            ...
            if row_num in matched_data:
                ...
                # Collect scores from Dimers and OddShark (they have direct scores)
                for source in ['dimers', 'oddshark']:
                ...
                BASE_TOTAL_SCORE = 50  # Typical total score in college football
    ...
    # Save to output file in data/ncaaf/ (maintains order in Python 3.7+)
    output_file = config.get_data_path("chatgpt_matched.json", league="ncaaf")
...
if __name__ == "__main__":
    """Run the ChatGPT/Tavily prediction gatherer."""
    print("\n=== CHATGPT/TAVILY NCAAF PREDICTION GATHERER ===\n")
    
    try:
        success = run_chatgpt_ncaaf()
        if success:
