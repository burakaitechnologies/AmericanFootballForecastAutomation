"""Google Sheets reader for NFL game data."""
...
class NFLSheetsReader:
    """Read NFL team matchup data from Google Sheets."""
    
    def __init__(self, config: Optional[Config] = None):
        """
        Initialize NFL sheets reader.
        ...
        self.client = GoogleSheetsClient(self.config, read_only=True, sheet_id=nfl_sheet_id)
        logger.info("NFLSheetsReader initialized")
    ...
    def read_games_data(self, sheet_name: Optional[str] = None) -> list[SheetsGame]:
        ...
        Args:
            sheet_name: Sheet name (defaults to "NFL" for NFL data)
    ...
    def save_games_to_file(self, output_file: Optional[str] = None) -> SheetsOutput:
        ...
        Args:
            output_file: Output file path (defaults to data/nfl/games_scraped/sheets_games.json)
            
        Returns:
            SheetsOutput object
        """
        if output_file is None:
            output_file = self.config.get_games_scraped_path("sheets_games.json", league="nfl")
...
def main():
    """Main function to demonstrate NFLSheetsReader usage."""
    try:
        print("=== NFL SHEETS READER ===")
        print("[READER] Google Sheets Reader Operations for NFL")
        
        print("\n[STEP 1] Retrieving team data from Google Sheets...")
        reader = NFLSheetsReader()
        data = reader.save_games_to_file()
        
        print(f"[SUCCESS] Retrieved {data.total_games} games from Google Sheets")
        print(f"[OUTPUT] Output file: {reader.config.get_games_scraped_path('sheets_games.json', league='nfl')}")
        ...
        print(f"\n[INFO] Sheet ID: {reader.config.get_sheet_id(league='nfl')}")
