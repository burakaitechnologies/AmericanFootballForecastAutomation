"""Scraper for Dimers.com NCAAF predictions."""
...
class DimersScraper(BaseScraper):
    """Scraper for Dimers.com college football predictions."""
    
    def __init__(self, max_concurrent_requests: int = 100):
        """Initialize Dimers scraper."""
        super().__init__(max_concurrent_requests)
        self.base_url = "https://www.dimers.com"
        self.schedule_url = "https://www.dimers.com/bet-hub/cfb/schedule"
    ...
    async def get_game_urls(self) -> list[str]:
        ...
        # Look for game links with class "game-link"
        ...
        # Try alternative patterns if no game-link found
        if not game_urls:
            self.logger.info("Trying alternative link patterns...")
            all_links = soup.find_all('a', href=True)
            for link in all_links:
                href = link.get('href')
                if href and '/bet-hub/cfb/schedule/' in href and href != '/bet-hub/cfb/schedule':
                    full_url = urljoin(self.base_url, href)
    ...
    async def parse_game_data(self, url: str) -> dict:
        ...
        try:
            ...
            if page_title:
                ...
                # Look for pattern like "Brown vs. Pennsylvania Prediction"
                vs_match = re.search(r'^(.+?)\s+vs\.\s+(.+?)\s+Prediction', title_text, re.IGNORECASE)
                if vs_match:
                    away_team = vs_match.group(1).strip()
                    home_team = vs_match.group(2).strip()
                    teams = [away_team, home_team]
                    self.logger.debug(f"Extracted teams: {away_team} @ {home_team}")
            
            # Look for score-row div with predicted scores
            scores = []
            score_row = soup.find('div', class_='score-row')
            
            if score_row:
                score_divs = score_row.find_all('div', class_='score')
            ...
            if len(teams) >= 2 and len(scores) >= 2:
                ...
                self.logger.warning(f"Incomplete data for {url}: teams={len(teams)}, scores={len(scores)}")
...
async def main():
    ...
    async with DimersScraper(max_concurrent_requests=100) as scraper:
        ...
        output_file = config.get_games_scraped_path("dimers_games.json", league="ncaaf")
